...
void TextCustomModifierNG::SetPosition(Vector2f position)
{
    if (position_ == nullptr) {
        position_ = std::make_shared<RSProperty<Vector2f>>(position);
        AttachProperty(position_);
...
void TextCustomModifierNG::SetBrushColor(uint32_t color)
{
    if (color_ == nullptr) {
        color_ = std::make_shared<RSProperty<uint32_t>>(color);
        AttachProperty(color_);
...
void TextCustomModifierNG::SetFontSize(float size)
{
    if (size_ == nullptr) {
        size_ = std::make_shared<RSProperty<float>>(size);
        AttachProperty(size_);
...
void TextCustomModifierNG::SetText(std::string text)
{
    if (text_ == nullptr) {
        text_ = std::make_shared<RSProperty<std::string>>(text);
        AttachProperty(text_);
...
void TextCustomModifierNG::Draw(ModifierNG::RSDrawingContext& context) const
{
    if (text_ == nullptr)
        return;
    uint32_t colorInt = 0xff000000;
