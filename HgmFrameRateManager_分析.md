...
sequenceDiagram
    ...
    HgmFrm->>HgmFrm: HandleScreenEvent(id)
...
## HandleScreenPowerStatus, HandleScreenRectFrameRate, HandleScreenEvent 逻辑分析
...
**HandleScreenEvent** (hgm_frame_rate_manager.cpp:966)
- 处理屏幕切换事件
- 更新当前屏幕ID、LTPO状态
- 构建屏幕名称（包含活动区域信息）
- 调用屏幕帧率处理
...
classDiagram
    ...
    HandleScreenEvent ..> isLtpo_ : 使用
    HandleScreenEvent ..> lastCurScreenId_ : 使用
    HandleScreenEvent ..> curScreenId_ : 使用
    HandleScreenEvent ..> activeRectScreenId_ : 使用
    HandleScreenEvent ..> activeRect_ : 使用
    HandleScreenEvent ..> isLtpoScreenStrategyId_ : 间接使用
    HandleScreenEvent ..> curScreenStrategyId_ : 间接使用
    HandleScreenEvent ..> curScreenDefaultStrategyId_ : 间接使用
```
...
三个函数协同工作，`HandleScreenEvent`是核心处理函数，被其他两个函数调用，负责更新屏幕状态并触发帧率策略更新。
