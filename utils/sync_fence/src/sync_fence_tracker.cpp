...
bool SyncFenceTracker::CheckGpuSubhealthEventLimit()
{
    ...
    if (gpuSubhealthEventNum == 0 || tm.tm_yday > gpuSubhealthEventDay) {
        ...
        gpuSubhealthEventNum++;
        HILOG_DEBUG(LOG_CORE, "%{public}" PRIu32 "event of %{public}" PRIu32 "day", gpuSubhealthEventNum, gpuSubhealthEventDay);
        return true;
    }
    HILOG_DEBUG(LOG_CORE, "%{public}" PRIu32 "event exceed %{public}" PRIu32 "day", gpuSubhealthEventNum, gpuSubhealthEventDay);
    return false;
}
...
inline double SyncFenceTracker::GetDoubleValue(const std::string& fileName)
{
    ...
    ss >> parsedVal;
    HILOG_DEBUG(LOG_CORE, "GetValue parsedVal is %f", parsedVal);
    return parsedVal;
}
    ...
    fenceIndex = fencesSignaled_.load();
    {
        ...
        if (threadName_.compare("Acquire Fence") == 0) {
            unit64_t startTimestamp = static_cast<unit64_t>(
                std::chrano::duration_cast<std::chrono::milliseconds>(
