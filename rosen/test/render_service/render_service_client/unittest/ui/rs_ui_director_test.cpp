...
{
    std::shared_ptr<RSUIDirector> director = RSUIDirector::Create(nullptr);
...
{
    std::shared_ptr<RSUIDirector> director1 = RSUIDirector::Create(nullptr);
    ASSERT_NE(director1->GetRSUIContext(), nullptr);
    std::shared_ptr<RSUIDirector> director2 = RSUIDirector::Create(nullptr);
...
HWTEST_F(RSUIDirectorTest, DestroyTest002, TestSize.Level1)
{
    ...
    {
        std::shared_ptr<RSUIDirector> childDirector = RSUIDirector::Create(nullptr);
...
{
    auto director = RSUIDirector::Create(nullptr);
    auto uiContext = director->GetRSUIContext();
    ASSERT_NE(uiContext, nullptr);
    uiContext->SetUITaskRunner([](const std::function<void()>& task, uint32_t delay) { task(); });
