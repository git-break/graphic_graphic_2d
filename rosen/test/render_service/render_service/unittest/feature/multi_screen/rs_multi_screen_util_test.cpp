 ...
 * @tc.desc: Test HandleVirtualExtendScreen with IsMultiSurfaceExtendMode true to hit SetDirtyInfo
 * @tc.type: FUNC
 * @tc.require: issue23778
 */
HWTEST_F(RSMultiScreenUtilTest, HandleVirtualExtendScreenTest008, TestSize.Level1)
{
    ...
    auto processor = std::make_shared<RSUniRenderVirtualProcessor>();
    params->screenProperty_.Set<ScreenPropertyType::SCREEN_STATUS>(
        static_cast<uint32_t>(VIRTUAL_SCREEN_PLAY));
    RSUniRenderThread::Instance().InitGrContext();
    processor->InitForRenderThread(*screenDrawable_, RSUniRenderThread::Instance().GetRenderEngine());

    // Enable multi-surface extend mode to exercise SetDirtyInfo branch
...
/**
 * @tc.name: HandleVirtualExtendDisplayTest003
 * @tc.desc: Test HandleVirtualExtendDisplay with multi-surface extend mode reaches DrawExpandDisplay
 * @tc.type: FUNC
 * @tc.require: #I9NVOG
 */
HWTEST_F(RSMultiScreenUtilTest, HandleVirtualExtendDisplayTest003, TestSize.Level1)
{
    auto& uniParam = RSUniRenderThread::Instance().GetRSRenderThreadParams();
    EXPECT_NE(uniParam, nullptr);
    auto params = static_cast<RSLogicalDisplayRenderParams*>(displayDrawable_->GetRenderParams().get());
    ...
    // Enable multi-surface extend mode for DrawExpandDisplay
    processor->needsOffscreenRender_ = true;
