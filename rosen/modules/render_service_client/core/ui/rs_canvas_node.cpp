...
void RSCanvasNode::FinishRecording()
{
    ...
    uint16_t modifierType = 0;
#ifdef MODIFIER_NG
        modifierType = static_cast<uint16_t>(drawContentLast_ ? ModifierNG::RSModifierType::FOREGROUND_STYLE
...
void RSCanvasNode::DrawOnNode(RSModifierType type, DrawFunc func)
{
    ...
    uint16_t modifierType = 0;
#ifdef MODIFIER_NG
        modifierType = static_cast<uint16_t>(ModifierTypeConvertor::ToModifierNGType(type));
    #else
        modifierType = static_cast<uint16_t>(type);
    #endif
    std::unique_ptr<RSCommand> command =
        std::make_unique<RSCanvasNodeUpdateRecording>(GetId(), recording, modifierType);
