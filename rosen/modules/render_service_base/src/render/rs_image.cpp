    ...
    const Drawing::Canvas& canvas, const Drawing::SamplingOptions& sampling) const
{
    ...
    auto sx = rectForDrawShader.GetWidth() / src_.GetWidth();
    auto sy = rectForDrawShader.GetHeight() / src_.GetHeight();
    auto tx = rectForDrawShader.GetLeft() - src_.GetLeft() * sx;
    auto ty = rectForDrawShader.GetTop() - src_.GetTop() * sy;
