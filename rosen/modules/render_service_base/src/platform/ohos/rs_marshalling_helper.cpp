...
bool RSMarshallingHelper::Marshalling(Parcel& parcel, const std::shared_ptr<EmitterConfig>& val)
{
    ...
    // success = success && Marshalling(parcel, val->size_.width_);
    // success = success && Marshalling(parcel, val->size_.height_);
    // success = success && Marshalling(parcel, val->imageFit_);
    return success;
}

bool RSMarshallingHelper::Unmarshalling(Parcel& parcel, std::shared_ptr<EmitterConfig>& val)
{
    bool success = Unmarshalling(parcel, val->emitRate_);
    success = success && Unmarshalling(parcel, val->emitShape_);
    success = success && Unmarshalling(parcel, val->position_.x_);
    success = success && Unmarshalling(parcel, val->position_.y_);
    success = success && Unmarshalling(parcel, val->emitSize_.x_);
    success = success && Unmarshalling(parcel, val->emitSize_.y_);
    success = success && Unmarshalling(parcel, val->particleCount_);
    success = success && Unmarshalling(parcel, val->lifeTime_);
    success = success && Unmarshalling(parcel, val->type_);
    success = success && Unmarshalling(parcel, val->radius_);
    // success = success && Unmarshalling(parcel, val->image_);
    // success = success && Unmarshalling(parcel, val->size_.width_);
    // success = success && Unmarshalling(parcel, val->size_.height_);
    // success = success && Unmarshalling(parcel, val->imageFit_);
    return success;
}
...
{
    return Unmarshalling(parcel, val->velocityValue_.start_) && Unmarshalling(parcel, val->velocityValue_.end_);
        Unmarshalling(parcel, val->velocityAngle_.start_) && Unmarshalling(parcel, val->velocityAngle_.end_);
}
...
bool RSMarshallingHelper::Unmarshalling(Parcel& parcel, std::shared_ptr<RenderParticleParaType<float>>& val)
{
    ...
    if (val->updator_ == ParticleUpdator::RANDOM) {
        ...
        for (size_t i = 0; i < valChangeOverLifeSize; i++) {
            ...
            auto change = std::make_shared<ChangeInOverLife<float>>(fromValue, toValue, startMillis,endMillis, interpolator); //需不需要加make_shared
...
bool RSMarshallingHelper::Unmarshalling(Parcel& parcel, std::shared_ptr<RenderParticleColorParaType>& val)
{
    ...
    if (val->updator_ == ParticleUpdator::RANDOM) {
        ...
        for (size_t i = 0; i < valChangeOverLifeSize; i++) {
            ...
            success = success && Unmarshalling(parcel, endMillis);
            std::shared_ptr<RSInterpolator> interpolator(RSInterpolator::Unmarshalling(parcel));
            auto change = std::make_shared<ChangeInOverLife<Color>>(fromValue, toValue, startMillis,endMillis, interpolator);
            valChangeOverLife.push_back(change);
        }
    }
    if (success) {
        val = std::make_shared<RenderParticleColorParaType>(colorVal, updator, redRandom, greenRandom, blueRandom, valChangeOverLife);
    }
    return success;
}
...
{
    bool success = Unmarshalling(parcel, val->emitterConfig_);
    success = success && Unmarshalling(parcel, val->velocity_);
    success = success && Unmarshalling(parcel, val->acceleration_.accelerationValue_);
    success = success && Unmarshalling(parcel, val->acceleration_.accelerationAngle_);
    success = success && Unmarshalling(parcel, val->color_);
    success = success && Unmarshalling(parcel, val->opacity_);
    success = success && Unmarshalling(parcel, val->scale_);
    success = success && Unmarshalling(parcel, val->spin_);
    return success;
}
