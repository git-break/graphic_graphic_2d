...
public:
    ...
    virtual ErrCode GetDisplayVCPFeature(ScreenId id, uint8_t vcpCode,
        uint16_t& currentValue, uint16_t& maximumValue, int32_t& errorCode) = 0;

    virtual ErrCode SetDisplayVCPFeature(ScreenId id, uint8_t vcpCode, uint16_t currentValue) = 0;
