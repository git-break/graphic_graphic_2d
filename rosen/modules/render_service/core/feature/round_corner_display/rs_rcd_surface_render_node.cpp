...
Drawing::ColorType RSRcdSurfaceRenderNode::PixelFormatToDrawingColorType(PixelFormat pixelFormat)
{
    switch (pixelFormat) {
...
Drawing::AlphaType RSRcdSurfaceRenderNode::AlphaTypeToDrawingAlphaType(AlphaType alphaType)
{
    switch (alphaType) {
...
void RSRcdSurfaceRenderNode::DrawRsRCDLayer(RSPaintFilterCanvas& canvas, const std::shared_ptr<RSLayer>& layer)
{
    ...
    if (!BindPixelMapToDrawingImage(canvas, rcdLayer) || rcdLayer->GetCacheImage() == nullptr) {
        RS_LOGE("RSRcdSurfaceRenderNode::DrawRsRCDLayer BindPixelMapToDrawingImage error");
        return;
    }
    Drawing::Brush brush;
...
bool RSRcdSurfaceRenderNode::BindPixelMapToDrawingImage(Drawing::Canvas& canvas,
...
bool RSRcdSurfaceRenderNode::StoreWithoutDMA(Drawing::Canvas& canvas,
    const std::shared_ptr<RSRenderSurfaceRCDLayer>& layer, RSRcdSurfaceRenderNode::PixelMapPtr& pixelMap)
{
    if (!pixelMap) {
        return false;
    }
    ...
    RS_LOGE("RSRcdSurfaceRenderNode::StoreWithoutDMA BuildFromBitmap error");
    return false;
}
