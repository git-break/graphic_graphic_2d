...
static constexpr std::array descriptorCheckList = {
    ...
    static_cast<uint32_t>(RSIClientToServiceConnectionInterfaceCode::GET_DISPLAY_VCP_FEATURE),
    static_cast<uint32_t>(RSIClientToServiceConnectionInterfaceCode::SET_DISPLAY_VCP_FEATURE),
    ...
    uint32_t code, MessageParcel& data, MessageParcel& reply, MessageOption& option)
{
    ...
    switch (code) {
        ...
        case static_cast<uint32_t>(RSIClientToServiceConnectionInterfaceCode::GET_DISPLAY_VCP_FEATURE): {
            ScreenId id{INVALID_SCREEN_ID};
            uint8_t vcpCode{0};
            if (!data.ReadUint64(id) || !data.ReadUint8(vcpCode)) {
                RS_LOGE("RSClientToServiceConnectionStub::GET_DISPLAY_VCP_FEATURE Read parcel failed!");
                ret = ERR_INVALID_DATA;
                break;
            }
            ...
            int32_t errorCode{0};
            if (GetDisplayVCPFeature(id, vcpCode, currentValue, maximumValue, errorCode) != ERR_OK ||
                !reply.WriteUint16(currentValue) || !reply.WriteUint16(maximumValue) ||
                !reply.WriteInt32(errorCode)) {
                RS_LOGE("RSClientToServiceConnectionStub::GET_DISPLAY_VCP_FEATURE Write failed!");
                ret = ERR_INVALID_REPLY;
            }
            break;
        }
        case static_cast<uint32_t>(RSIClientToServiceConnectionInterfaceCode::SET_DISPLAY_VCP_FEATURE): {
            ScreenId id{INVALID_SCREEN_ID};
            uint8_t vcpCode{0};
            uint16_t currentValue{0};
            if (!data.ReadUint64(id) || !data.ReadUint8(vcpCode) || !data.ReadUint16(currentValue)) {
                RS_LOGE("RSClientToServiceConnectionStub::SET_DISPLAY_VCP_FEATURE Read parcel failed!");
                ret = ERR_INVALID_DATA;
                break;
            }
            if (SetDisplayVCPFeature(id, vcpCode, currentValue) != ERR_OK) {
                RS_LOGE("RSClientToServiceConnectionStub::SET_DISPLAY_VCP_FEATURE Write failed!");
                ret = ERR_INVALID_REPLY;
            }
            break;
        }
