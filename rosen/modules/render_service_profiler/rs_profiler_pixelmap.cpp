...
bool RSProfiler::MarshalPixelMap(Parcel& parcel, const std::shared_ptr<Media::PixelMap>& map)
{
    ...
    if (pixelCheckEnabled && !Media::PixelMapRecordParcel::MarshallingPixelMapForRecord(parcel, *(map.get()))) {
...
Media::PixelMap* RSProfiler::UnmarshalPixelmapNstd(Parcel& parcel)
{
    ...
    if (IsRecordAbortRequested()) {
        return Media::PixelMapRecordParcel::UnmarshallingPixelMapForRecord(parcel);
    }
    ...
    if (map && !Media::PixelMapRecordParcel::ReadMemInfoFromParcel(parcel, memory, error, nullptr)) {
        delete map;
        return nullptr;
    }
    ...
    std::function<int(Parcel& parcel, std::function<int(Parcel&)> readFdDefaultFunc)> readSafeFdFunc)
{
    ...
    if (pixelCheckEnabled) {
        return UnmarshalPixelmapNstd(parcel);
    }
